...
class AttentionLoss(nn.Module):
    def __init__(self, equ_lambda, layers): # equ_lambda = equalization weight (hyper parameter for )
        super().__init__()
...
def tokenize_function(examples):        #creating randomly negative or positive sentences sg's
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_path", type=str, help="Output path", required=True)

    args = parser.parse_args()

    print(vars(args))

    layers = list(range(args.layers))   # default = 12, Layers used in debiasing


    with open(args.def_word_filepath, 'r') as f:
        groups = json.load(f)["biases"]     # filepath to the JSON file containing definition tuples per bias type


    with open(args.vocab_filepath, 'r') as f:
        vocab = f.read().split('\n')    # filepath to vocabulary used for negative examples
